...
const CFG = {
  ...
  SESSION_QUESTIONS: [
    ...
    'Seberapa fokus kamu tadi?',
    'Apa yang berhasil kamu selesaikan?',
    'Masih ada energi untuk sesi berikutnya? ⚡',
    'Gimana perasaanmu sekarang?',
    'Perlu istirahat sebentar?',
    'Sudah bangga sama dirimu sendiri belum? 🌟',
  ],

  // GANTI/TAMBAH placeholder textarea acak di sini:
  SESSION_PLACEHOLDERS: [
    'Tulis apa yang kamu accomplish tadi...',
    'Gimana konsentrasinya? Ada gangguan?',
    'Catatan singkat untuk sesi berikutnya...',
    'Perasaan setelah sesi ini... (opsional)',
...
function transitionPopup(fromStep, toStep) {
  ...
  fromEl.style.cssText = 'opacity:0;transform:translateY(-20px) scale(0.95);transition:opacity 0.25s ease,transform 0.25s ease;';
...
function setMode(mode) {
  ...
  const computedAccent = getComputedStyle(D.html).getPropertyValue('--accent').trim();
...
function loadActivityData() {
  try { return JSON.parse(localStorage.getItem(CFG.LS_ACTIVITY_KEY)) || {}; }
  catch { return {}; }
}
...
function recordActivity(mode, minutes) {
  ...
  if (!data[today]) data[today] = { pomodoro: 0, 'short-break': 0, 'long-break': 0 };
...
function updateStats() {
  ...
  days7.forEach(d => {
    ...
    if (day) totalMin += (day.pomodoro || 0) + (day['short-break'] || 0) + (day['long-break'] || 0);
  });

  let streak = 0;
  for (let i = days7.length - 1; i >= 0; i--) {
    const day = actData[days7[i].key];
    if (day?.pomodoro > 0) { streak++; } else break;
  }

  S.totalSessions = parseInt(localStorage.getItem(CFG.LS_SESSIONS_KEY) || '0', 10);
...
function openSessionModal() {
  const imgSrc = CFG.MODAL_IMAGES[Math.floor(Math.random() * CFG.MODAL_IMAGES.length)];
...
function generateBeep() {
  try {
    ...
    [[880, 0, 0.15], [1100, 0.18, 0.12], [880, 0.32, 0.2]].forEach(([freq, start, dur]) => {
...
function bindEvents() {
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.code === 'Space') { e.preventDefault(); handleMainButton(); }
    if (e.code === 'Escape' && !D.modal.classList.contains('is-hidden')) closeModalNoSave();
  });
}
